...
class RobDataParallelPPOActor(BasePPOActor):
    ...
    def update_policy(self, data: DataProto):
        ...
        select_keys = ['responses', 'input_ids', 'attention_mask', 'pixel_values', 'old_log_probs', 'advantages', 'finish_step', 'importance_weights']
        batch = data.select(batch_keys=select_keys).batch
        ...
        for batch_idx, data in enumerate(dataloader):
            ...
            for test_idx, data in enumerate(micro_batches):
                ...
                # 获取重要性采样权重
                importance_weights = data.get('importance_weights', None)
                if importance_weights is not None:
                    importance_weights = importance_weights.to(data['input_ids'].device)
                else:
                    # 如果没有提供权重，使用均匀权重
                    importance_weights = torch.ones(data['responses'].size(1), device=data['input_ids'].device)
                importance_weights = importance_weights.unsqueeze(1).expand(batch_size, response_length)
                ...
                for i in range(0, traj_len, int(traj_len/traj_split_num)):
                    ...
                    importance_weights_tmp = importance_weights[:, slice_id: next_slice_id]
                    ...
                    # }
                    # torch.save(data_dict, f'/share/home/u16023/jinyiyang/experiments/SimpleVLA-RL/outputs/tensors_b{batch_idx}_t{test_idx}_s{slice_id}.pt')
                    ...
                    pg_loss, pg_clipfrac, ppo_kl = core_algos.compute_policy_loss_weighted(old_log_prob=old_log_prob_tmp,
                                                                            log_prob=log_prob,

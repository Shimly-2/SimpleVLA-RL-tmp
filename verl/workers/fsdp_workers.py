...
class RobActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if self._is_actor:
            ...
            assert self.config.actor.ppo_mini_batch_size % self.config.actor.ppo_micro_batch_size == 0, f"normalized ppo_mini_batch_size {self.config.actor.ppo_mini_batch_size} should be divisible by ppo_micro_batch_size {self.config.actor.ppo_micro_batch_size}"
            assert self.config.actor.ppo_mini_batch_size // self.config.actor.ppo_micro_batch_size > 0, f"normalized ppo_mini_batch_size {self.config.actor.ppo_mini_batch_size} should be larger than ppo_micro_batch_size {self.config.actor.ppo_micro_batch_size}"
        if self._is_rollout:

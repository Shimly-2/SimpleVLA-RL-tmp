...
#         # 优先级采样（可选）
#         self.priorities = deque(maxlen=config.data.max_buffer_size)
#         self.enable_priority = False
        
#     def add_batch(self, batch_data: DataProto, priority: float = 1.0):
#         """添加批次数据到缓冲区"""
#         with self.not_full:
...
#     def sample_batch(self, batch_size: int) -> Optional[List[DataProto]]:
#         """从缓冲区采样批次"""
#         with self.not_empty:
#             while len(self.buffer) < self.config.data.min_buffer_size:
...
#             else:
#                 samples = self._uniform_sample(batch_size)
...
#     def _priority_sample(self, batch_size: int) -> List[DataProto]:
#         """优先级采样"""
#         import numpy as np
...
#         """检查缓冲区是否准备好进行训练"""
#         return self.size() >= self.config.data.min_buffer_size
    
    
# class AsyncDataGenerator:
#     def __init__(self, trainer, actor_rollout_wg, tokenizer, train_dataloader):
...
#         # 异步队列用于存储生成的批次
#         self.generated_batches_queue = asyncio.Queue(maxsize=100)
...
#         try:
#             # 准备批次参数
#             batch_size = self.trainer.config.data.train_batch_size
#             n_samples = self.trainer.config.data.n_samples
...
#             # 生成足够的批次数据
#             consecutive_none_count = 0  # 连续获取None的计数
#             max_consecutive_none = 3    # 最大连续None次数
...
#                         print("Data exhausted, breaking from generation loop")
#                         break
#                     await asyncio.sleep(0.1)  # 稍等再试
#                     continue
#                 else:
#                     consecutive_none_count = 0  # 重置计数
                
#                 # 异步生成序列
#                 try:
#                     roll_batch, batch_metrics = await self._process_batch_async(
#                         batch_dict, buffer_batch, n_samples
#                     )
                    
#                     if roll_batch is not None:
#                         valid_batch.append(roll_batch)
...
#             'metrics': dict(self.generation_metrics),
#             'average_gen_time': np.mean(self.generation_metrics.get('timing/gen', [0])),
...
class AsyncDataGenerator:
    def __init__(self, trainer, actor_rollout_wg, tokenizer, train_dataloader, config):
        ...
        self.safe_executor = SafeExecutorWrapper(
            ThreadPoolExecutor(max_workers=2, thread_name_prefix="dataloader")
        )
        self.generation_thread = None
        self.stop_event = threading.Event()
        self.generation_metrics = defaultdict(list)
    ...
    async def _generate_batch_async(self) -> Optional[DataProto]:
        """异步生成单个批次数据 - 正确处理n_samples"""
        # try:
        # 收集rollout数据，直到达到 batch_size * n_samples
        valid_batch = []
        buffer_batch = []
        ...
        # 生成数据直到达到目标数量: batch_size * n_samples
        target_rollouts = self.batch_size * self.n_samples
        
        while len(valid_batch) < target_rollouts:
            # 获取下一个原始批次数据
            batch_dict = await self._get_next_batch_async()
            if batch_dict is None:
            ...
            # 处理这个批次，生成n_samples个rollout
            rollout_batch, batch_metrics = await self._process_single_batch_with_nsamples(
                batch_dict, buffer_batch
            )
            
            if rollout_batch is not None:
                # 注意：rollout_batch已经包含了这个批次的所有n_samples个rollout
                valid_batch.append(rollout_batch)
            ...
            # 检查是否已经收集够了
            current_rollouts = sum(len(batch.batch) for batch in valid_batch)
            if current_rollouts >= target_rollouts:
                break
        ...
        if valid_batch:
            ...
            print(f"Generated batch with {len(final_batch.batch)} rollouts "
                    f"(target: {target_rollouts})")
    ...
    async def _process_single_batch_with_nsamples(self, batch_dict: dict, buffer_batch: List) -> Tuple[Optional[DataProto], dict]:
        """处理单个批次，为每个样本生成n_samples个rollout"""
        start_time = time.time()
        ...
        try:
            ...
            # 关键：为每个原始样本创建n_samples个副本
            # 这里实现了对每个数据rollout n_samples次
            batch_lst = []
            for i in range(len(newbatch)):
                # 对第i个样本，创建n_samples个副本
                single_sample = newbatch[i:i + 1]
                for _ in range(self.n_samples):
                    batch_lst.append(single_sample)
            
            print(f"Created {len(batch_lst)} rollouts from {len(newbatch)} original samples "
            ...
            # 合并结果：每个原始样本对应n_samples个rollout
            roll_batch = DataProto.concat(batch_lst)
            roll_batch = roll_batch.union(gen_batch_output)
            
            # 验证结果
            expected_rollouts = len(newbatch) * self.n_samples
            actual_rollouts = len(roll_batch.batch)
            print(f"Expected {expected_rollouts} rollouts, got {actual_rollouts}")
            
            # 计算指标
            gen_time = time.time() - start_time
            batch_metrics = {
                'timing/gen': gen_time,
                'rollouts_generated': actual_rollouts,
            ...
            print(f"Process batch with n_samples error: {e}")
            return None, {}
...
class AsyncExperienceBuffer:
    ...
    def sample_batch(self, batch_size: int) -> Optional[DataProto]:
        """从缓冲区采样批次 - 采样batch_size个rollout组成训练批次"""
        with self.not_empty:
            # 等待足够的rollout用于训练
            min_rollouts_needed = batch_size
            
            while len(self.buffer) < min_rollouts_needed:
                if not self.not_empty.wait(timeout=1.0):
                    return None  # 超时返回None
            
            # 采样batch_size个rollout
            sampled_rollouts = self._uniform_sample(batch_size)
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            while True:
                ...
                while not self.experience_buffer.is_ready():
                    ...
                    print(f"Buffer size: {buffer_size}/{self.config.data.min_buffer_size}")
                    
                # 从缓冲区采样批次
                valid_batch = self.experience_buffer.sample_batch(batch_size)

    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            while True:
                ...
                # Relabel
                batch_indexs = [idx for idx in range(0, len(valid_batch), n_samples)]
                id2score, id2mean, id2std, id2diffs = [], [], [], []
                for i in batch_indexs:
                    tmp = []
                    for samples in range(i, i + n_samples):
                        tmp.append(float(valid_batch[samples].batch['complete'].tolist()))
                    id2score.append(tmp)
                ...
                for i in batch_indexs:
                    for samples in range(i, i + n_samples):
                        if not valid_batch[samples].batch["complete"]: # and random.uniform(0, 1) > 0.5:
                            valid_batch[samples].batch["complete"] = torch.tensor([id2mean[int(i//n_samples)]])
                            valid_batch[samples].batch['finish_step'] = torch.tensor([random.randint(256, 511)]) # 2, 256, 511 / first, normal, last

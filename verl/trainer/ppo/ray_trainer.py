...
def _select_by_advantage(advantages, threshold=0, absolute=True):
    # advantages = batch.batch["advantages"][:,0]
    advantages = torch.cat(advantages, dim=0)
    advantages = advantages[:,0]
    # print("advantages select:", advantages)
    if absolute:
    ...
    # print(f"select samples by advantage threshold={threshold}: {len(selected_indices)}/{selected_mask.shape[0]}")
    ...
    def get_generation_stats(self) -> dict:
        ...
        for timing_key in ['timing/gen', 'timing/verify', 'timing/acc&trunc_filter']:
            ...
            if times:
                ...
                base_stats[timing_key] = np.mean(times) * len(times)
        ...
        for kl_key in ['critic/kl', 'critic/kl_coeff']:
            times = self.generation_metrics.get(kl_key, [])
            if times:
                base_stats[kl_key] = np.mean(times)
                
        for reward_key in ['train_reward/verifier', 'train_reward/reward_all']:
            times = self.generation_metrics.get(reward_key, [])
            if times:
                base_stats[reward_key] = np.mean(times)
    ...
    async def _generate_batch_async(self) -> Optional[DataProto]:
        ...
        # 不再等待完整批次，而是持续生成并添加
            ...
            while generation_count < target_generations:
                ...
                if self.trainer.experience_buffer.is_ready():
                    all_metrics = defaultdict(list)
                    self.traffic_sign = True
                    print("Stop generation data until update is finish")
                ...
                reward_tensor_dict, reward_metrics = self.reward_fn(processed_batch)
                processed_batch.batch['token_level_scores'] = reward_tensor_dict['all']
                
                for k, v in reward_metrics.items():
                    all_metrics['train_reward/' + k].append(v)
                
                for k,v in reward_tensor_dict.items():
                    processed_batch.batch[k]=v
                    
                processed_batch, kl_metrics = apply_kl_penalty(processed_batch,
                                                    kl_ctrl=self.trainer.kl_ctrl,
                                                    kl_penalty=self.trainer.config.algorithm.kl_penalty,
                                                    action_token_len=self.trainer.config.actor_rollout_ref.model.action_token_len, 
                                                    action_chunks_len=self.trainer.config.actor_rollout_ref.model.action_chunks_len,)
                
                all_metrics.update(kl_metrics)
                
                # compute advantages, executed on the driver process
                processed_batch = compute_advantage(processed_batch,
                                            self.trainer.config.algorithm.gamma,
                                            self.trainer.config.algorithm.lam,
...
class AsyncExperienceBuffer:
    def __init__(self, config):
        ...
        self.max_version_diff = getattr(config.data, 'max_version_diff', 1)
        self.quality_threshold = getattr(config.data, 'quality_threshold', 0.3)
        
        self.buffer_metrics = defaultdict(list)
        self.sample_step = 0
        ...
        if sampling_strategy == 'adv_priority':
            ...
            for version_diff, items in version_groups.items():
                ...
                for idx, mean_val in items:
                    final_scores[idx] = base_score + 0.5 * mean_val  # mean作为微调
    ...
    def sample_batch_with_important_sampling(self, batch_size: int = None, sampling_strategy='adv_priority', epsilon=1e-4) -> Optional:
        ...
        with self.not_empty:
            ...
            advantages = [self.buffer[i]["data"].batch["advantages"] for i in range(len(self.buffer))]
            ...
            high_adv_indexs = _select_by_advantage(advantages, threshold=self.config.algorithm.high_adv_threshold, absolute=True)
            print("high_adv_indexs",high_adv_indexs, len(high_adv_indexs))
            ...
            # print("advantages",advantages)
            print("id2diffs",id2diffs, "current_policy_version", self.current_policy_version)
            # print("id2mean",id2mean)
            # print("id2std",id2std)
            
            if self.config.data.relabel:
                print("Relabel sample batch")
                self._relabel_rollout(batch_indexs, id2mean)
            
            on_policy_indexs = []
            off_policy_indexs = []
            off_policy_indexs_dense = []
            for i in batch_indexs:
                if id2diffs[int(i//self.n_samples)] == 0:
                    on_policy_indexs.append(i)
                else:
                    off_policy_indexs.append(i)
                    for j in range(i, i + self.n_samples):
                        off_policy_indexs_dense.append(j)
                    
            high_adv_off_policy_indexs = [item for item in high_adv_indexs if Counter(off_policy_indexs_dense)[item] > 0]
            
            # 1. 计算所有样本的采样概率
            id2mean_off_policy = [id2mean[int(i//self.n_samples)] for i in off_policy_indexs]
            id2std_off_policy = [id2std[int(i//self.n_samples)] for i in off_policy_indexs]
            id2diffs_off_policy = [id2diffs[int(i//self.n_samples)] for i in off_policy_indexs]
            sampling_probs = self._compute_sampling_probabilities(sampling_strategy, id2mean_off_policy, id2std_off_policy, advantages, id2diffs_off_policy)
            print("on_policy_indexs",on_policy_indexs, len(on_policy_indexs))
            print("off_policy_indexs",off_policy_indexs, len(off_policy_indexs))
            # print("off_policy_indexs_dense",off_policy_indexs_dense, len(off_policy_indexs_dense))
            # print("high_adv_off_policy_indexs",high_adv_off_policy_indexs, len(high_adv_off_policy_indexs))
            # print("total_rollouts_needed",total_rollouts_needed)
            
            if len(high_adv_off_policy_indexs) > 0:
                # return None
            
                id2mean_on_policy = [id2mean[int(i//self.n_samples)] for i in on_policy_indexs]
                id2std_on_policy = [id2std[int(i//self.n_samples)] for i in on_policy_indexs]
                id2diffs_on_policy = [id2diffs[int(i//self.n_samples)] for i in on_policy_indexs]
                sampling_probs = self._compute_sampling_probabilities(sampling_strategy, id2mean_on_policy, id2std_on_policy, advantages, id2diffs_on_policy)
                
                if len(high_adv_off_policy_indexs) % self.n_samples != 0:
                    nums = len(high_adv_off_policy_indexs) // self.n_samples
                    
                    if nums*self.n_samples > int((1 - self.config.data.on_data_ratio) * total_rollouts_needed):
                        high_adv_off_policy_indexs = high_adv_off_policy_indexs[:int((1 - self.config.data.on_data_ratio) * total_rollouts_needed)]
                    else:
                        high_adv_off_policy_indexs = high_adv_off_policy_indexs[:nums*self.n_samples]
                
                if total_rollouts_needed - len(high_adv_off_policy_indexs) <= len(on_policy_indexs) * self.n_samples and total_rollouts_needed - len(high_adv_off_policy_indexs) > 0:
                    on_policy_sampled_indices = self._sample_indices_by_probability(
                        on_policy_indexs, sampling_probs, int(total_rollouts_needed / self.n_samples) - int(len(high_adv_off_policy_indexs) / self.n_samples)
                    )
                else:
                    on_policy_sampled_indices = on_policy_indexs
                # sampled_indices = list(sampled_indices)
                # high_adv_off_policy_indexs.extend(sampled_indices)
                # sampled_indices = np.array(high_adv_off_policy_indexs)
            
                # 2. 根据概率采样索引
                # try:
                #     sampled_indices = self._sample_indices_by_probability(
                #         off_policy_indexs, sampling_probs, int(total_rollouts_needed / self.n_samples) - len(on_policy_indexs)
                #     )
                # except:
                #     sampled_indices = []
                # sampled_indices = list(sampled_indices)
                # sampled_indices.extend(on_policy_indexs)
                ...
                on_policy_sampled_indices = self._sample_indices_by_probability(
                    on_policy_indexs, sampling_probs, int(total_rollouts_needed / self.n_samples)
                )
                # sampled_indices = on_policy_indexs
            print("high_adv_off_policy_indexs",high_adv_off_policy_indexs, len(high_adv_off_policy_indexs))
            print("on_policy_sampled_indices",on_policy_sampled_indices, len(on_policy_sampled_indices))
            
            # 3. 计算重要性权重
            # importance_weights = self._compute_importance_weights(
            #     sampled_indices, sampling_probs
            # )
            importance_weights = torch.ones(total_rollouts_needed)
            # print("importance_weights",importance_weights)
            # import ipdb;ipdb.set_trace()
            on_policy_mask = []
            off_policy_mask = []
            if len(high_adv_off_policy_indexs) > 0:
                for i in high_adv_off_policy_indexs:
                    on_policy_mask.append(False)
                    off_policy_mask.append(True)
            for i in on_policy_sampled_indices:
                for samples in range(i, i + self.n_samples):
                    on_policy_mask.append(True)
                    off_policy_mask.append(False)
            ...
            #             on_policy_mask.append(False)
            #             off_policy_mask.append(True)
            on_policy_mask = torch.tensor(on_policy_mask, dtype=torch.bool)
            off_policy_mask = torch.tensor(off_policy_mask, dtype=torch.bool)
            
            # 4. 提取采样的数据
            sampled_items = []
            if len(high_adv_off_policy_indexs) > 0:
                for i in high_adv_off_policy_indexs:
                    sampled_items.append(self.buffer[i])
            for i in on_policy_sampled_indices:
                for samples in range(i, i + self.n_samples):
            ...
            training_batch.batch["on_policy_mask"] = on_policy_mask
            training_batch.batch["off_policy_mask"] = off_policy_mask
            
            # 7. 计算和打印统计信息
            self._log_sampling_stats(importance_weights, sampling_strategy, on_policy_sampled_indices)
            print(f"  On policy data ratio: {on_policy_mask.sum()/on_policy_mask.shape[-1]}")
            print(f"  Off policy data ratio: {off_policy_mask.sum()/off_policy_mask.shape[-1]}")
            
            self.buffer_metrics["buffer/on_policy_data_ratio"].append(on_policy_mask.sum()/on_policy_mask.shape[-1])
            self.buffer_metrics["buffer/off_policy_data_ratio"].append(off_policy_mask.sum()/off_policy_mask.shape[-1])
    ...
    def _relabel_rollout(self, batch_indexs, id2mean):
        target = int(256 * (1 + 1 / self.config.data.relabel_step * self.sample_step))
        ...
        for i in batch_indexs:
            for samples in range(i, i + self.n_samples):
                if not self.buffer[samples]["data"].batch["complete"]:
                    ...
                    self.buffer[samples]["data"].batch['finish_step'] = torch.tensor([random.randint(target, 511)])
    ...
    def is_ready(self) -> bool:
        ...
        try:
            advantages = [self.buffer[i]["data"].batch["advantages"] for i in range(len(self.buffer))]
            high_adv_indexs = _select_by_advantage(advantages, threshold=1, absolute=True)        
            high_adv_off_policy_indexs = [item for item in high_adv_indexs if Counter(off_policy_indexs_dense)[item] > 0]
            high_adv_off_policy_nums = len(high_adv_off_policy_indexs) // self.n_samples * self.n_samples
        except:
            high_adv_off_policy_nums = 0
        # return len(self.buffer) >= max(min_rollouts_for_training, min_for_batch) and len(on_policy_indexs) * self.n_samples >= random.randint(int(min_rollouts_for_training / 2), min_rollouts_for_training) and len(on_policy_indexs) * self.n_samples + high_adv_off_policy_nums >= max(min_rollouts_for_training, min_for_batch)
        return len(self.buffer) >= max(min_rollouts_for_training, min_for_batch) and \
               len(on_policy_indexs) * self.n_samples >= int(min_rollouts_for_training * self.config.data.on_data_ratio) and \
               len(on_policy_indexs) * self.n_samples + high_adv_off_policy_nums >= max(min_rollouts_for_training, min_for_batch)
               
    def get_buffer_stats(self) -> dict:
        """获取buffer信息"""
        base_stats = dict()
        
        for buffer_key, value in self.buffer_metrics.items():
            base_stats[buffer_key] = np.mean(value)
...
def compute_advantage(data: DataProto, gamma, lam, adv_estimator, config):
    ...
    if adv_estimator == 'rloo':
        ...
        steps_expanded = steps.unsqueeze(0).expand(data.batch['responses'].size(0), -1)
        response_mask = steps_expanded < finish_step.unsqueeze(1)  # (batch_size, traj_len)
        on_policy_mask = data.batch['on_policy_mask']
        # on_policy_mask = ~prefix_mask.any(-1)
        # from .mix_core_alg import compute_grpo_outcome_advantage_split
        advantages, returns = core_algos.compute_grpo_outcome_advantage_split(
    ...
    def fit(self):
        ...
        for epoch in range(self.config.trainer.total_epochs):
            ...
            while True:
                ...
                with Timer(name='step', text="{name}: {seconds:.1f} seconds") as timer:
                    ...
                    roll_batch_to_add = self.experience_buffer.sample_batch_with_important_sampling(batch_size, sampling_strategy='version_only_with_mean_tiebreak', epsilon=1e-6)
                    ...
                    buffer_metrics = self.experience_buffer.get_buffer_stats()
                    ...
                    with Timer(name='old_log_prob', text="{name}: {seconds:.1f} seconds") as timer:
                        batch = self.actor_rollout_wg.compute_log_prob(batch)
                    metrics['timing/old_log_prob'] = timer.last
                    ...
                    if self.config.trainer.critic_warmup <= global_steps:
                        ...
                        # import ipdb;ipdb.set_trace() # actor_output.meta_info['metrics']['actor/on_pg_loss']  actor_output_metrics['actor/on_pg_loss']
                ...
                metrics['timing/step'] = timer.last

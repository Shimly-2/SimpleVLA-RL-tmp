...
class RobRewardManager():
    ...
    def verify(self, data): # dict_keys(['trial_id', 'task_id', 'attention_mask', 'old_log_probs', 'input_ids', 'responses', 'finish_step', 'complete', 'pixel_values'])
        completes = data.batch['complete'].tolist()
        batch_size = data.batch['responses'].size(0) # torch.size([32, 64, 56]) 56 = 7 * 8, traj_len = 64
        assert len(completes) == batch_size
        score = [float(item) for item in completes]
        format = [1.0 for _ in range(len(completes))]
        
        # Add step-based reward
        # finish_step = data.batch['finish_step'].tolist()
        # step_score = [1.0 - float(item)/512.0 for item in finish_step]
        # new_score = []
        # for s1, s2 in zip(score, step_score):
        #     new_score.append(s1 + s2)
        # data.batch['acc'] = torch.tensor(new_score, dtype=torch.float32, device=data.batch['responses'].device)
